...
const { getSegmentsFromBuild, getPointsOfSegment, getBuildFromFilePath, getSubsegmentsOfSegment, renumberSegments } = require('../common.js');
...
function AdjustPower (build, heat) {
  ...
  const highTempPoints = new Set(); const lowTempPoints = new Set();
  ...
  for (const segment of getSegmentsFromBuild(build)) {
    ...
    const isCloseToHighHeat = false; const isCloseToLowHeat = false;

    // Going in 1mm intervals, split the segment into sections that are thermally okay, too high, and too low
    let tooLow = new Set(); let tooHigh = new Set(); let okay = new Set();
    ...
    for (const subsegment of getSubsegmentsOfSegment(segment, SAMPLING_INTERVAL)) {
      for (const point in highTempPoints) {
        if (distance({ x: subsegment.x1, y: subsegment.y1 }, point) < MAX_DISTANCE ||
      ...
      for (const subsegment of segments) {
        for (const segment in newSegments) {
          if (subsegment.x1 === segment.x1 && subsegment.y1 === segment.y1) {
            segment.x1 = subsegment.x2; segment.y1 = subsegment.y2;
          } else if (subsegment.x2 === segment.x2 && subsegment.y2 === segment.y2) {
            segment.x2 = subsegment.x1; segment.y2 = subsegment.y1;
          } else {
